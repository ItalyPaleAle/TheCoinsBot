    ...
    */
    init() {
        ...
        this._websocket.on('close', (data) => {
            this._pino.warning({type: 'websocket', message: 'Websocket closed unexpectedly', data: data})
            ...
            const interval = setInterval(() => {
                if (!this._websocket.socket) {
                    ...
                    if (count % 30 === 0) {
                        ...
                        this._pino.fatal({type: 'websocket', message: `Attempting to re-connect for the ${count} time. It has been ${time_since} seconds since we lost connection.`})
                    }
                    this._websocket.connect()
                }
                else {
