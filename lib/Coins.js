...
class Coins {
    ...
    */
    init() {
        ...
        this._websocket.on('close', (data) => {
            this._pino.warn({type: 'coins', message: 'Websocket closed unexpectedly', data: data})
                ...
                const interval = setInterval(() => {
                    if (!this._websocket.socket) {
                        ...
                        if (count % 30 === 0) {
                            ...
                            this._pino.fatal({type: 'coins', message: `Attempting to reconnect to websocket for the ${count} time. It has been ${time_since} seconds since we lost connection.`})
                        }
                        this._websocket.connect()
                    }
                    else {
                        this._pino.info({type: 'coins', message: 'Reconnected to websocket'})
    ...
    */
    price(product) {
        ...
        // Return prices
        const currentPrice = this._prices[product].price
        const openPrice = this._prices[product].open_24h
        return {
            price: parseFloat(currentPrice),
            volume_24h: parseInt(this._prices[product].volume_24h, 10),
            change_percent: ((currentPrice - openPrice) / openPrice * 100).toFixed(2),
            low_24h: parseFloat(this._prices[product].low_24h),
            high_24h: parseFloat(this._prices[product].high_24h),
